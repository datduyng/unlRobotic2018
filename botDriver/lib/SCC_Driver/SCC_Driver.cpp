...
//TODO: This function need to be modified.
...
void toCalibratedPoint( float x, float y, float z){
  // this function using toPoint() function.
  // Using calibration constraint to Use toPoint() function more accurately
...
void toFetal(void){
  ...
  targetConfig[0] = 90; // base
  targetConfig[1] = 180;// should
  targetConfig[2] = 100; //  50degree offset from last year . // 150 degree originally.  // elbow
  targetConfig[3] = 0;// wrist .
...
void commandMode(void){
  ...
  Serial.println("3 - Enter Position( With calibration) ");
	...
	while(1){
		...
		if(cmd==2){
			Serial.print("\n");
			sendPosition(-1);}// send coordinate with out calibration}

    if(cmd==3){
      Serial.print("\n");
      sendPosition(1);// send coordinate with calibration.
    }
		if(cmd==4){
...
void sendPosition(int isCalibrated){
	// Prompts User for coordinates of point to which cutter will be positioned
  // take one argument. if isCalibrated > 0 then Send calibrate coordinate
  // else send normal coor dinate
	float point[3];
  ...
  if(isCalibrated > 0){
    // mainly for debug and calibration
